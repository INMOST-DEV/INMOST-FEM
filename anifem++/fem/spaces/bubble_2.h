...
#ifndef CARNUM_FEM_ANIINTERFACE_SPACES_BUBBLE_2_H
#define CARNUM_FEM_ANIINTERFACE_SPACES_BUBBLE_2_H

#include "common.h"

namespace Ani{
    template<>
    struct Operator<IDEN, FemFix<FEM_B2>> {
    ...
    template<>
    struct Operator<GRAD, FemFix<FEM_B2>> {
        using Nfa = std::integral_constant<int, 6>;
        ...
        apply(AniMemory<ScalarType, IndexType> &mem, ArrayView<ScalarType> &U) {
            ...
            assert(mem.extraR.size >= 3 * 6 * q && "Not enough memory for operator initialization in mem.extraR");
            (void) wsz; (void) nfa;
            DenseMatrix<ScalarType> GRAD_B2(mem.extraR.data, 3, 6 * q, mem.extraR.size);
            ScalarType GRAD_P1[3 * 4] = {-1, -1, -1, 1, 0, 0, 0, 1, 0, 0, 0, 1};
            std::array<unsigned char, 6> I = {0, 0, 0, 1, 1, 2}, J = {1, 2, 3, 2, 3, 3};
            for (std::size_t n = 0; n < q; ++n) {
                for (IndexType i = 0; i < 6; ++i) {
                    auto s1 = mem.XYL.data[I[i] + 4 * n], s2 = mem.XYL.data[J[i] + 4 * n];
                    for (IndexType d = 0; d < 3; ++d)
                        GRAD_B2.data[d + 3 * (i + 6 * n)] = 4 * (GRAD_P1[d + 3 * J[i]] * s1 + GRAD_P1[d + 3 * I[i]] * s2);
                }
            }
                    ...
                    for (std::size_t n = 0; n < q; ++n)
                        for (IndexType k = 0; k < Dim::value; ++k) {
                            ...
                            for (IndexType j = 0; j < 3; ++j)
                                s += mem.PSI.data[j + 3 * (k + 3 * r)] * GRAD_B2.data[j + 3 * (i + 6 * n)];
    ...
    template<>
    struct Dof<FemFix<FEM_B2>>{
        static inline DofT::UniteDofMap Map(){
            return DofT::UniteDofMap(DofT::DofSymmetries({0}, {1, 0}));
        }
        ...
        static inline void interpolate(const Tetra<const double>& XYZ, const EvalFunc& f, std::array<ArrayView<>, FUSION> udofs, int idof_on_tet, void* user_data = nullptr, uint max_quad_order = 5){
            std::array<double, Operator<IDEN, FemFix<FEM_B2>>::Dim::value * FUSION> mem = {0};
    ...
    using Bubble2Space = GenSpace<Bubble, FEM_B2>;
}
#endif //CARNUM_FEM_ANIINTERFACE_SPACES_BUBBLE_2_H
