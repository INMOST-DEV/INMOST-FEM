...
/**
 * Here we show how to write the code FEM problem based on memory-dependend templated finite element spaces 
...
int main(int argc, char* argv[]){
    ...
    PlainMemory<> mem_req;
    {
        mem_req = fem3Dtet_memory_requirements<Operator<GRAD, UFem>, Operator<GRAD, UFem>>(quad_order);
    ...
    std::function<void(const double**, double*, double*, double*, int*, void*)> local_assembler =
        ...
        fem3Dtet<Operator<GRAD, UFem>, Operator<GRAD, UFem>, DfuncTraits<TENSOR_SCALAR, false>>( XYZ, D_tensor, A, mem, order );

        // elemental right hand side vector <F, P1>
        fem3Dtet<Operator<IDEN, FemFix<FEM_P0>>, Operator<IDEN, UFem>, DfuncTraits<TENSOR_SCALAR, true>>( XYZ, F_tensor, F, mem, order );
    ...
    //define assembler
    Assembler discr(m);
    discr.SetMatRHSFunc(GenerateElemMatRhs(local_assembler, UNF, UNF, mem_req.dSize, mem_req.iSize));
    {
